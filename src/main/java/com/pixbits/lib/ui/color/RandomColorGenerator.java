...
public class RandomColorGenerator implements ColorGenerator
{
  private class ColorInfo
  {
    ...
    public ColorInfo(Range hueRange, Range saturationRange,
        Range brightnessRange, List<Range> lowerBounds)
    {
  ...
  public class RandomColor
  {
    ...
    public int randomColor(int value, SaturationType saturationType,
    ...
    public java.awt.Color randomColor(Color color)
    {
      ...
      return java.awt.Color.getHSBColor(hue / 255.0f, saturation / 255.0f, brightness / 255.0f);
    }

    public java.awt.Color[] random(Color color, int count)
    {
      if (count <= 0)
      {
        throw new IllegalArgumentException("count must be greater than 0");
      ...
      java.awt.Color[] colors = new java.awt.Color[count];
      for (int i = 0; i < count; i++)
      {
    ...
    private int pickSaturation(int hue, SaturationType saturationType,
        Luminosity luminosity)
    {
      return pickSaturation(getColorInfo(hue), saturationType, luminosity);
    }

    private int pickSaturation(Color color, SaturationType saturationType,
        Luminosity luminosity)
    {
    ...
    private int pickSaturation(ColorInfo colorInfo,
        SaturationType saturationType, Luminosity luminosity)
    {
    ...
    private int pickBrightness(Color color, int saturation,
        Luminosity luminosity)
    {
      ColorInfo colorInfo = colors.get(color.name());
    ...
    private int pickBrightness(ColorInfo colorInfo, int saturation,
        Luminosity luminosity)
    {
      int min = getMinimumBrightness(colorInfo, saturation), max = 100;
    ...
    private int getMinimumBrightness(ColorInfo colorInfo, int saturation)
    {
      ...
      for (int i = 0; i < lowerBounds.size() - 1; i++)
      {
        ...
        int s1 = lowerBounds.get(i).start, v1 = lowerBounds.get(i).end;

        if (i == lowerBounds.size() - 1)
        {
          break;
        }
        int s2 = lowerBounds.get(i + 1).start, v2 = lowerBounds.get(i + 1).end;

        if (saturation >= s1 && saturation <= s2)
        {

          float m = (v2 - v1) / (float) (s2 - s1), b = v1 - m * s1;
    ...
    private ColorInfo getColorInfo(int hue)
    {
      ...
      for (String key : colors.keySet())
      {
        ...
        if (colorInfo.getHueRange() != null
            && colorInfo.getHueRange().contain(hue))
        {
          return colorInfo;
        }
      }
    ...
    private int randomWithin(Range range)
    {
      return (int) Math.floor(
          range.start + random.nextDouble() * (range.end + 1 - range.start));
    }

    public void defineColor(String name, Range hueRange,
        List<Range> lowerBounds)
    {
      ...
      colors.put(name, new ColorInfo(hueRange, new Range(sMin, sMax),
          new Range(bMin, bMax), lowerBounds));
    }
    ...
    private void loadColorBounds()
    {
      ...
      defineColor(Color.MONOCHROME.name(), null, lowerBounds1);
      ...
      defineColor(Color.RED.name(), new Range(-26, 18), lowerBounds2);
      ...
      defineColor(Color.ORANGE.name(), new Range(19, 46), lowerBounds3);
      ...
      defineColor(Color.YELLOW.name(), new Range(47, 62), lowerBounds4);
      ...
      defineColor(Color.GREEN.name(), new Range(63, 178), lowerBounds5);
      ...
      defineColor(Color.BLUE.name(), new Range(179, 257), lowerBounds6);
      ...
      defineColor(Color.PURPLE.name(), new Range(258, 282), lowerBounds7);
      ...
      defineColor(Color.PINK.name(), new Range(283, 334), lowerBounds8);
    }
  }
  ...
  private final RandomColor gen = new RandomColor();
  private Luminosity luminosity = null;
  private SaturationType saturation = null;
  
  public RandomColorGenerator() { }
  
  public RandomColorGenerator(SaturationType saturation, Luminosity luminosity)
  { 
    this.luminosity = luminosity;
  ...
  {
    //return new java.awt.Color(gen.randomColor(0, saturation, luminosity));
    return gen.randomColor(Color.RED);
  }
}
